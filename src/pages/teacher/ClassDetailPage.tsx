...
const ATTENDANCE_STYLES: Record<string, string> = {
 PRESENT: "bg-success-container text-on-success-container border border-success-container",
 ...
 EXCUSED: "bg-surface-container-low text-on-surface-variant border border-border",
}
  ...
  switch (tab) {
    ...
    return (
     <div className="space-y-md">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-sm">
       <h3 className="font-headline-sub text-headline-sub text-primary">Enrolled Students ({students.length})</h3>
       <div className="relative w-full sm:w-64">
        ...
        <input
         ...
         className="w-full pl-9 pr-3 py-1.5 rounded-full text-label-md font-body-md bg-surface-container-low focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary"
        ...
        <table className="w-full text-left border-collapse">
         <thead>
          <tr className="bg-surface-container-low border-b border-outline-variant">
           <th className="py-3 px-4 font-meta text-meta uppercase text-on-surface-variant tracking-[0.02em]">Student</th>
           <th className="py-3 px-4 font-meta text-meta uppercase text-on-surface-variant tracking-[0.02em] hidden sm:table-cell">Email</th>
           <th className="py-3 px-4 font-meta text-meta uppercase text-on-surface-variant tracking-[0.02em]">Status</th>
           <th className="py-3 px-4 font-meta text-meta uppercase text-on-surface-variant tracking-[0.02em] text-right">Action</th>
          </tr>
         </thead>
           ...
           <tr key={s.id} className="hover:bg-surface-container-low transition-colors">
            <td className="py-3 px-4">
             <div className="flex items-center gap-3">
              ...
              <Link to={`/students/${s.id}`} className="font-body-md text-label-md font-medium text-on-surface hover:text-primary transition-colors">{s.name}</Link>
             </div>
            </td>
            <td className="py-3 px-4 text-label-md text-on-surface-variant hidden sm:table-cell">{s.email}</td>
            <td className="py-3 px-4">
             <span className="inline-flex items-center px-2 py-0.5 rounded-full text-label-sm font-medium bg-success-container text-on-success-container border border-success-container">Enrolled</span>
            </td>
    ...
    return (
     <div className="space-y-md">
      <div className="flex items-center justify-between">
       <h3 className="font-headline-sub text-headline-sub text-primary">Assignments</h3>
       <Button asChild className="flex items-center gap-2 px-4 py-2 h-auto rounded-full bg-primary text-primary-foreground font-body-medium text-label-md hover:bg-primary/90 transition-colors">
      ...
      {assignmentsData.length === 0 ? (
       ...
       <div className="space-y-sm">
        {assignmentsData.map((a) => (
         <Link key={a.id} to={`/assignments/${a.id}`} className="block bg-surface-container-lowest rounded p-md flex items-center justify-between group hover:border-primary hover:shadow-card-hover transition-all">
          <div className="flex items-center gap-3">
           <div className="w-11 h-11 rounded bg-surface-container flex items-center justify-center text-primary shrink-0">
            <span className="material-symbols-outlined">description</span>
           </div>
           <div>
            <h4 className="font-label-md text-label-md text-primary">{a.title}</h4>
   ...
   case "attendance":
    return (
     <div className="space-y-md">
      <div className="flex items-center justify-between">
       <h3 className="font-headline-sub text-headline-sub text-primary">Attendance</h3>
       <Button asChild className="flex items-center gap-2 px-4 py-2 h-auto rounded-full bg-primary text-primary-foreground font-body-medium text-label-md hover:bg-primary/90 transition-colors">
      ...
      {attendanceRecords.length === 0 ? (
       ...
       <div className="overflow-x-auto bg-surface-container-lowest rounded-lg ">
        <table className="w-full text-label-md font-body-md">
         <thead>
          <tr className="bg-surface-container-low border-b border-outline-variant">
         ...
         <tbody>
          {students.map((s: { id: string; name: string }) => (
           <tr key={s.id} className="border-b border-outline-variant last:border-none">
            <td className="px-4 py-3 font-body-medium text-label-md text-on-surface whitespace-nowrap">{s.name}</td>
            {dates.map((date) => {
  ...
  return (
   <div className="flex items-center justify-center h-full p-xl">
    <div className="text-center w-full">
     ...
     <h2 className="font-headline-md text-headline-md text-primary mb-sm">Class not found</h2>
     <p className="font-body-md text-on-surface-variant mb-lg">{error instanceof Error ? error.message : "Failed to load class"}</p>
     <Link to="/classes" className="bg-primary text-primary-foreground px-lg py-sm rounded-lg font-label-md inline-block">Back to Classes</Link>
    </div>
   </div>
  )
 }
 ...
 return (
  <div className="min-h-full bg-surface">
   <div className="mx-auto w-full max-w-6xl p-gutter pb-24 md:pb-0">
    <Link to="/classes" className="inline-flex items-center gap-1 text-on-surface-variant hover:text-primary font-body-md text-label-md mb-4 transition-colors">
    ...
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-md">
     <div>
      <h2 className="font-headline-xl text-headline-xl text-primary">{cls.name}</h2>
     ...
     <div className="flex items-center gap-2">
      <Button asChild className="flex items-center gap-2 px-4 py-2 h-auto rounded-full bg-primary text-primary-foreground font-body-medium text-label-md hover:bg-primary/90 transition-colors shadow-card">
    ...
    <div className="bg-surface-container-lowest rounded-xl shadow-card overflow-hidden flex flex-col">
     <div className="border-b border-outline-variant px-6 flex overflow-x-auto shrink-0 hide-scrollbar">
      {TABS.map((tab) => (
       <button
        key={tab.id}
        onClick={() => setActiveTab(tab.id)}
        className={`py-4 px-2 mr-6 font-body-medium text-label-md whitespace-nowrap transition-colors ${
