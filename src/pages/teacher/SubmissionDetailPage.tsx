      ...
      {sub.chunks && sub.chunks.length > 0 ? (
       <div className="space-y-sm">
        {sub.chunks.map((chunk, i) => (
         <div key={chunk.id} className="bg-surface-container-low rounded-lg p-md ">
      ...
      {(sub.status === "REVIEW_READY" || sub.status === "CONFIRMED") && mergedScores.length > 0 && (
       <div className="space-y-md">
        {mergedScores.map((score) => (
         <div key={score.id} className="bg-surface-container-low rounded-lg p-md space-y-sm">
          <div className="flex items-start justify-between gap-sm">
           <div className="flex-1 min-w-0">
            <p className="font-label-md text-label-md text-on-surface">{score.criterion?.description ?? "Criterion"}</p>
            <p className="font-label-sm text-label-sm text-on-surface-variant">Max {score.criterion?.maxPoints ?? 0} pts</p>
           </div>
           <span className="font-headline-sm text-headline-sm text-primary whitespace-nowrap">{score.pointsAwarded}/{score.criterion?.maxPoints ?? 0}</span>
          </div>
           ...
           <div className="space-y-sm pt-sm border-t border-outline-variant">
            <div>
             ...
             <input
              ...
              className="w-full bg-surface-container-lowest rounded-lg px-3 py-2 font-body-md text-body-md text-on-surface form-input-focus"
             />
            </div>
            <div>
             ...
             <textarea
              ...
              className="w-full bg-surface-container-lowest rounded-lg px-3 py-2 font-body-md text-body-md text-on-surface form-input-focus resize-none"
             />
            </div>
           </div>
          )}
        ...
        <div className="bg-primary-container/10 rounded-lg p-md border border-primary-container/20">
         <div className="flex items-center justify-between">
          <span className="font-label-md text-label-md text-primary font-semibold">Total</span>
