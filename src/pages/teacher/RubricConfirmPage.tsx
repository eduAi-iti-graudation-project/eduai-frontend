 ...
 if (rubric.isConfirmed) {
  return (
   <div className="flex-1 p-margin-desktop max-w-3xl mx-auto w-full">
    <div className="bg-surface-container-lowest rounded-lg p-xl space-y-lg">
     ...
     <div className="stagger-enter space-y-md">
      {rubric.criteria.map((c) => (
       <div key={c.id} className="flex items-center justify-between p-md bg-surface-container-low rounded-lg">
        ...
        <span className="font-label-md text-label-md text-primary font-semibold ml-4">{c.maxPoints} pts</span>
       </div>
      ))}
     </div>
     <div className="flex justify-between items-center p-md bg-surface-container-high rounded-lg">
      <span className="font-label-md text-label-md text-on-surface font-semibold">Total</span>
  ...
  <div className="flex-1 p-margin-desktop max-w-3xl mx-auto w-full">
   <div className="bg-surface-container-lowest rounded-lg p-xl space-y-lg">
    ...
    <div className="space-y-4">
     <div className="space-y-2">
      ...
      <input
       ...
       className="w-full bg-surface-container-lowest focus:border-primary focus:ring-2 focus:ring-primary/20 rounded-lg p-4 font-body-md text-body-md transition-all"
      />
     </div>

     {criteria.map((c) => (
      <div key={c.id} className="flex items-start gap-3 p-4 bg-surface-container-lowest rounded-lg">
       <div className="flex-1 space-y-2">
        <textarea
         value={c.description}
         onChange={(e) => updateCriterion(c.id, "description", e.target.value)}
         rows={2}
         className="w-full bg-surface-container-lowest focus:border-primary focus:ring-2 focus:ring-primary/20 rounded-lg p-3 font-body-md text-body-md transition-all resize-none"
        ...
        <div className="flex items-center gap-2">
         <input
          ...
          className="w-24 bg-surface-container-lowest focus:border-primary focus:ring-2 focus:ring-primary/20 rounded-lg p-2 font-body-md text-body-md text-center transition-all"
          placeholder="pts"
         />
    ...
    <div className="bg-accent border border-primary-container rounded-lg p-md flex items-start gap-3">
     ...
     <div>
      <p className="font-label-md text-label-md text-on-surface font-semibold">This action cannot be undone</p>
    ...
    <Button
     ...
     className="w-full h-auto py-3 rounded-full font-headline-md text-headline-md font-semibold transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 bg-primary text-primary-foreground hover:bg-primary/90"
    >
