          ...
          {isLoading ? (
            ...
            <div className="space-y-3 max-w-4xl mx-auto">
              {alerts.data?.map((alert) => (
                <div
                  key={alert.id}
                  className="tactile-card rounded-[24px] bg-surface-container-lowest p-4 flex items-start gap-4"
                >
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${
                    alert.status === "NEW" ? "bg-red-100 text-red-600" : "bg-surface-container-low text-on-surface-variant"
                  }`}>
                    <span className="material-symbols-outlined text-[20px]">
                      {typeIcons[alert.type] ?? "notifications_active"}
                    </span>
                  </div>

                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`font-label-sm text-label-sm px-2 py-0.5 rounded-full ${statusColors[alert.status] ?? "bg-gray-100 text-gray-800"}`}>
                        {alert.status}
                      </span>
