        ...
        <header ref={builderTopRef} className="flex flex-col lg:flex-row lg:items-center justify-between gap-md border-b border-border pb-3">
          ...
          {!assignmentId && (
            <div className="w-full lg:w-[320px]">
              ...
              >
                <SelectTrigger className="h-auto w-full gap-2 rounded-md bg-surface-container-lowest border border-outline-variant px-3 py-2 text-label-md text-on-surface focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 shadow-none">
        ...
        <section className="bg-surface-container-lowest rounded-lg border border-outline-variant shadow-sm overflow-hidden">
          <div className="flex items-center justify-between gap-md border-b border-outline-variant px-md py-3">
            <div className="flex items-center gap-sm flex-wrap">
              <h2 className="font-headline-md text-headline-md text-on-surface">Rubric Builder</h2>
              <Badge
                variant="outline"
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <Button
                ...
                className="h-auto px-md py-1.5 rounded-md border border-outline-variant bg-surface-container-lowest text-on-surface-variant hover:bg-surface-container hover:text-on-surface font-label-sm text-label-sm disabled:opacity-50"
              >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 divide-y lg:divide-y-0 lg:divide-x divide-outline-variant">
            ...
            <div className="lg:col-span-5 p-md space-y-md">
              <div className="space-y-base">
                ...
                <input
                  ...
                  className="w-full px-md py-2 bg-surface-container-lowest border border-outline-variant rounded-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all font-body-md text-on-surface"
              ...
              <div className="space-y-md">
                {manualCriteria.map((criterion) => (
                  <div key={criterion.id} className="rounded-md border border-outline-variant bg-surface-container-low p-sm space-y-sm">
                    <div className="flex items-center gap-2">
                      <input
                        value={criterion.name}
                        onChange={(e) => updateRow(criterion.id, "name", e.target.value)}
                        className="flex-1 min-w-0 px-3 py-1.5 bg-surface-container-lowest border border-outline-variant rounded-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all font-body-md"
                      ...
                      <input
                        ...
                        className="w-16 text-center px-2 py-1.5 bg-surface-container-lowest border border-outline-variant rounded-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all font-body-md"
                        placeholder="pts"
                      />
                      <Button
                        type="button"
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full px-3 py-1.5 bg-surface-container-lowest border border-outline-variant rounded-md focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all font-body-sm text-on-surface resize-none"
                    ...
                    <div key={`ai-${i}`} className="rounded-md border border-primary border-dashed bg-accent/40 p-sm flex items-center gap-sm">
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <Button
                          ...
                          className="h-auto px-2.5 py-1.5 rounded-md bg-primary text-white text-xs font-medium"
                        >
                          Accept
                        </Button>
                        <Button
                          ...
                          variant="outline"
                          className="h-auto px-2.5 py-1.5 rounded-md border border-outline-variant bg-surface-container-lowest text-on-surface-variant text-xs font-medium hover:bg-surface-container"
                        >
                          Dismiss
                        </Button>
                      </div>
            ...
            <div className="lg:col-span-7 p-md">
              <div className="flex items-center justify-between mb-md">
                <h3 className="font-headline-md text-headline-md text-on-surface">Live Preview</h3>
                <div className="flex items-center gap-2">
                  {selectedRubricId && !rubrics.data?.find((r) => r.id === selectedRubricId)?.isConfirmed && (
                    <Button
                      ...
                      className="h-auto px-md py-1.5 rounded-md bg-primary text-on-primary font-label-sm text-label-sm hover:bg-primary/90 disabled:opacity-50"
                    >
                      Confirm & Publish
                    </Button>
                  )}
              ...
              {isLoading ? (
                    ...
                    return (
                      <>
                        <div className="px-md py-3 border-b border-outline-variant bg-surface-container-low flex items-center justify-between">
                          <div>
                            <h4 className="font-headline-md text-headline-md text-on-surface">{rubric.title}</h4>
                            <p className="font-label-sm text-label-sm text-on-surface-variant">
                              {rubric.criteria.length} criteria · {rubric.criteria.reduce((s, c) => s + c.maxPoints, 0)} total pts
                            </p>
                          </div>
                          {rubric.isConfirmed ? (
                            <Badge className="bg-primary text-white px-2 py-0.5 rounded-md font-label-sm text-label-sm border-0">Confirmed</Badge>
                          ) : (
                        ...
                        <ul className="divide-y divide-outline-variant">
                          {rubric.criteria.map((c) => (
                            <li key={c.id} className="flex items-center justify-between px-md py-2.5">
                              ...
                              <span className="font-label-sm text-label-sm text-primary font-bold ml-4">{c.maxPoints} pts</span>
                            </li>
                          ))}
                        </ul>
                ...
                <div className="rounded-lg border border-outline-variant overflow-hidden">
                  <div className="px-md py-3 border-b border-outline-variant bg-surface-container-low">
                    <h4 className="font-headline-md text-headline-md text-on-surface">{title || "Untitled Rubric"}</h4>
        ...
        {rubrics.data && rubrics.data.length > 0 && (
          <section>
            <div className="flex items-end justify-between mb-sm">
              <h3 className="font-headline-md text-headline-md text-on-surface">Saved Rubrics</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-md">
              {rubrics.data.map((rubric) => (
                <div
                  key={rubric.id}
                  className={`text-left bg-surface-container-lowest rounded-lg border border-outline-variant shadow-sm p-md transition-all hover:shadow-md ${
                ...
                >
                  ...
                  <div className="mt-md grid grid-cols-2 gap-2">
                    <Button
                      ...
                      variant="outline"
                      className="border border-outline-variant bg-surface-container-lowest text-on-surface-variant hover:bg-surface-container hover:text-primary text-xs font-medium rounded-md px-2 py-1.5 h-auto"
                    >
                      View
                    </Button>
                    <Button
                      ...
                      className="rounded-md bg-primary text-on-primary text-xs font-medium hover:bg-primary/90 px-2 py-1.5 h-auto"
                    >
                      Use in Builder
                    </Button>
                    {resolvedAssignmentId && (
                      <Button
                        ...
                        disabled={createRubric.isPending}
                        className="col-span-2 rounded-md border border-outline-variant bg-surface-container-lowest text-on-surface-variant hover:bg-surface-container hover:text-primary text-xs font-medium px-2 py-1.5 h-auto disabled:opacity-50"
                      >
                        Copy to Assignment
                      </Button>
                    )}
