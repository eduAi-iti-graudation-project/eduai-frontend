...
export function CourseMaterialsTab({ courseId, offeringId, sectionName }: CourseMaterialsTabProps) {
 const { chapters, unassigned, create, update, remove, move, refetch } =
  useCourseMaterialChapters(courseId)
 ...
 const renderMaterial = (m: Material) => (
  <div
   ...
   className="group flex items-center justify-between gap-sm rounded-lg bg-surface-container-low px-sm py-2 /60 hover:border-primary/40 transition-colors cursor-grab active:cursor-grabbing"
  >
   ...
   <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
    ...
    <button
     ...
     className="p-1.5 rounded-full text-on-surface-variant hover:text-error hover:bg-surface-container transition-colors"
    >
 ...
 return (
  <div className="space-y-md">
   <div className="flex items-center justify-between">
    <h3 className="font-headline-sub text-headline-sub text-primary">
     Course Materials
    </h3>
    <Button
     type="button"
     onClick={() => setIsCreating((v) => !v)}
     className="flex items-center gap-2 px-4 py-2 h-auto rounded-full bg-primary text-primary-foreground font-body-medium text-label-md hover:bg-primary/90 transition-colors"
    >
   ...
   {isCreating && (
    <div className="flex items-center gap-sm bg-surface-container-lowest rounded-lg p-md ">
     ...
     <input
      ...
      className="flex-1 px-md py-sm rounded-lg bg-surface-container-low font-body-md text-body-md text-on-surface placeholder:text-on-surface-variant/60 focus:border-primary focus:outline-none"
      autoFocus
     />
     <Button
      type="button"
      ...
      className="h-auto px-md py-sm rounded-lg bg-primary text-primary-foreground font-label-md text-label-md hover:opacity-90 transition-colors disabled:opacity-50"
     >
      Create
     </Button>
    </div>
    ...
    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-md">
     <div className="flex-1">
      ...
      <p className="font-label-sm text-label-sm text-on-surface-variant mt-0.5">
       Materials are shared across every section of this course — they are
       added through {sectionName || "your section"} and appear for all
       students. Drop one or more PDFs anywhere here — or straight onto a
       chapter below.
      </p>
     </div>
     <div className="flex items-center gap-sm">
      <Button
       ...
       className="h-auto px-md py-2 rounded-lg text-on-surface font-label-md text-label-md hover:border-primary hover:text-primary transition-colors"
      >
     ...
     <div className="mt-md space-y-sm">
      <div className="flex flex-wrap gap-2">
       {queued.map((q, i) => (
        <div
         key={`${q.file.name}-${i}`}
         className="inline-flex items-center gap-2 rounded-lg bg-surface-container px-sm py-1.5 "
        >
      ...
      <Button
       ...
       className="h-auto px-md py-2 rounded-lg bg-primary text-primary-foreground font-label-md text-label-md hover:opacity-90 transition-colors disabled:opacity-50"
      >
   ...
   <div className="space-y-sm">
    {sortedChapters.map((chapter) => (
     ...
     >
      ...
      {dragChapter === chapter.id && (
       <div className="mx-md mb-sm px-md py-sm rounded-xl border-2 border-dashed border-primary bg-primary-fixed/10 text-center">
   ...
   <div className="space-y-sm">
    ...
    {unassigned.length > 0 ? (
     <div
      className="grid gap-1.5 p-md bg-surface-container-lowest rounded-lg "
      onDragOver={(e) => {
       e.preventDefault()
