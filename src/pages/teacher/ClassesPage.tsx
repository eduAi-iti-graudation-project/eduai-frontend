  ...
  <div className="min-h-full bg-surface">
   <div className="mx-auto flex max-w-6xl flex-col gap-md p-gutter pb-24 md:pb-0">
    ...
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-md">
     <div>
      <h2 className="font-headline-xl text-headline-xl text-primary">Your Sections</h2>
    ...
    <div className="flex flex-wrap items-center gap-sm">
     <div className="relative w-full">
      ...
      <input
       ...
       className="w-full pl-10 pr-4 py-2 bg-surface-container-lowest rounded-full text-label-md font-body-md text-on-surface placeholder:text-outline focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all"
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-md">
     {filtered.length === 0 && (
      <div className="col-span-full bg-surface-container-lowest rounded-lg border border-dashed border-outline-variant shadow-card p-md flex flex-col items-center justify-center text-center gap-sm py-xl">
       <span className="material-symbols-outlined text-[40px] text-outline">school</span>
       <h4 className="font-headline-md text-headline-md text-primary">No sections found</h4>
     ...
     {filtered.map((c) => (
      <div
       key={c.id}
       className="bg-surface-container-lowest rounded-lg shadow-card hover:shadow-card-hover transition-shadow relative group"
      >
       <div className="p-md border-b border-outline-variant flex justify-between items-start">
        <div className="min-w-0">
         <h3 className="font-headline-md text-headline-md text-primary truncate">{c.name}</h3>
       ...
       <div className="p-md">
        <div className="flex justify-between items-center mb-sm">
         ...
         <span className="px-2 py-0.5 rounded bg-primary text-primary-foreground font-label-sm text-label-sm ">
          Active
         </span>
        </div>
        <Link
         to={`/classes/${c.id}`}
         className="mt-md w-full flex items-center justify-center gap-1 px-3 py-2 rounded-md bg-surface-container-highest/40 text-on-surface hover:border-primary hover:bg-primary hover:text-on-primary transition-colors font-body-md text-body-md"
        >
         View Section
