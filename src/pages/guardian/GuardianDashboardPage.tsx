    ...
    <div className="stagger-enter grid grid-cols-1 md:grid-cols-2 gap-4">
     {dashboard.data.children.map((child) => (
      <Button
       ...
       className="w-full h-auto flex flex-col items-start justify-start gap-0 rounded-lg bg-background p-xl border border-border hover:border-primary-container/30 hover:shadow-card-hover hover:scale-[1.02] transition-all hover:bg-background"
      >
       <div className="flex items-start justify-between mb-4 w-full">
        ...
        {child.activeAlertCount > 0 && child.activeAlertId && (
         <Badge
          variant="outline"
          className="bg-primary-container text-on-primary-container font-label-sm text-label-sm px-sm py-0.5 rounded-full border-0 cursor-pointer"
          onClick={(e) => {
           e.stopPropagation()
       ...
       <div className="stagger-enter grid grid-cols-2 gap-4 w-full">
        <div className="bg-surface-container-low rounded-lg p-md text-center">
         ...
         <p className="font-headline-md text-headline-md text-on-surface">{child.overallAverage}%</p>
        </div>
        <div className="bg-surface-container-low rounded-lg p-md text-center">
         <p className="font-label-sm text-label-sm text-on-surface-variant">Attendance</p>
         <p className="font-headline-md text-headline-md text-on-surface">{child.attendanceRate}%</p>
        </div>
       </div>
      </Button>
     ))}
