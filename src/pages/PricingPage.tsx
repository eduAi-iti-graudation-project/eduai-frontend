...
function PlanLink({ to, children, featured }: { to: string; children: React.ReactNode; featured?: boolean }) {
 const cls = `inline-flex items-center justify-center rounded-full px-md py-2.5 font-label-md text-label-md transition-colors ${
  featured
 ...
 return (
  <div className="mx-auto w-full max-w-5xl p-xl">
   ...
   <header className="mb-lg text-center">
    <h1 className="font-headline-xl text-headline-xl text-primary mb-2">Pricing that grows with your school</h1>
   ...
   <div className="mb-xl grid grid-cols-1 gap-lg md:grid-cols-3">
    {TIERS.map((t) => (
     <div
      key={t.name}
      className={`flex flex-col rounded-xl border p-lg shadow-card ${
       t.featured ? "border-primary bg-primary text-primary-foreground shadow-card-hover" : "border-border bg-surface-container-lowest text-on-surface"
      }`}
     >
      <div className="flex items-center justify-between">
       <p className="font-label-md text-label-md tracking-[0.16em] uppercase opacity-80">{t.name}</p>
       {t.featured && <span className="rounded-full bg-background/25 px-3 py-1 font-label-sm text-label-sm">Most popular</span>}
      </div>
