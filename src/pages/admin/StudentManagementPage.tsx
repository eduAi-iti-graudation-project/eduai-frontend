...
const statusBadge: Record<string, string> = {
 ...
 LATE: "bg-secondary-fixed text-on-secondary-fixed-variant",
 EXCUSED: "bg-primary-fixed text-on-surface-variant",
}
         ...
         >
          ...
          <div className="flex flex-col items-end gap-1 shrink-0">
           <div className="flex items-center gap-1">
            {grade && (
             <span className="font-label-sm text-label-sm px-1.5 py-0.5 rounded-full bg-primary-fixed text-on-primary-fixed-variant font-semibold">
              {grade.name}
             </span>
            )}
            {flagCount > 0 && (
             <span className="inline-flex items-center gap-0.5 font-label-sm text-label-sm px-1.5 py-0.5 rounded-full bg-error-container text-on-error-container font-semibold">
      ...
      {!selectedStudent ? (
       ...
       <>
        ...
        {studentInsightsQ.data && studentInsightsQ.data.agentInsights.length > 0 && (
         <div className="rounded-lg bg-surface-container-lowest overflow-hidden">
          <div className="px-md py-3 border-b border-outline-variant flex items-center justify-between">
           <div>
            <h3 className="font-headline-sm text-headline-sm text-primary">AI overview</h3>
          ...
          <div className="divide-y divide-border">
           {studentInsightsQ.data.agentInsights.slice(0, 3).map((insight, i) => (
            <div key={`${insight.title}-${i}`} className="px-md py-3.5">
        ...
        {studentFlags.length > 0 && (
         <div className="rounded-lg bg-surface-container-lowest overflow-hidden">
          <div className="px-4 py-2.5 border-b border-outline-variant">
           <h3 className="font-headline-sm text-headline-sm text-primary">Active alerts</h3>
          </div>
          <div className="divide-y divide-border">
           {studentFlags.map((a) => (
            <div key={a.id} className="flex items-center gap-3 px-md py-3">
             <div className="flex-1 min-w-0">
        ...
        <div className="rounded-lg bg-surface-container-lowest overflow-hidden">
         <div className="px-md py-3 border-b border-outline-variant flex items-center justify-between">
          <div>
           <h3 className="font-headline-sm text-headline-sm text-primary">Confirmed grades</h3>
         ...
         {classSummaries.length === 0 ? (
          ...
          <div className="space-y-3 p-4">
           {classSummaries.map((cls) => (
            <div key={cls.id} className="rounded-md overflow-hidden">
                ...
                {cls.assignments.map((a) => (
                 <TableRow key={a.id} className="border-b border-outline-variant hover:bg-surface-container-low">
                  ...
                  <TableCell className="py-2 px-2 text-right font-body-md text-body-md">
                   <span className={cn("font-semibold", a.maxPossible > 0 && a.earned / a.maxPossible >= 0.5 ? "text-on-success-container" : "text-on-error-container")}>
        ...
        <div className="rounded-lg bg-surface-container-lowest overflow-hidden">
         <div className="px-md py-3 border-b border-outline-variant flex items-center justify-between flex-wrap gap-3">
          <div>
           <h3 className="font-headline-sm text-headline-sm text-primary">Attendance</h3>
           <p className="font-label-sm text-label-sm text-on-surface-variant mt-0.5">
            {attendanceStats.total} records · {attendanceStats.rate === null ? "—" : `${attendanceStats.rate}% present`}
           </p>
          </div>
          <Select value={attendanceFilter} onValueChange={setAttendanceFilter}>
           <SelectTrigger className="w-auto h-9 gap-1 rounded-md bg-surface-container-low px-3 font-label-md text-label-md">
        ...
        <div className="rounded-lg bg-surface-container-lowest p-md">
         ...
         <div className="relative mb-3">
          ...
          <Input
           ...
           className="w-full pl-10 pr-3 py-1.5 rounded-md font-body-md text-body-md bg-surface-container-low outline-none focus:border-primary h-9 focus-visible:ring-transparent focus-visible:ring-offset-0"
          />
         </div>
            ...
            <AlertDialogContent className="rounded-xl bg-surface-container-lowest max-w-[460px]">
             <AlertDialogHeader>
              ...
              <AlertDialogDescription className="space-y-2 font-body-md text-body-md text-on-surface-variant">
               <p>
                This <span className="font-semibold text-on-error-container">cannot be undone</span>. The student's
             ...
             <AlertDialogFooter className="mt-4">
              ...
              <AlertDialogAction
               ...
               className={cn(
                ...
                canExpel ? "bg-error" : "bg-error/40 cursor-not-allowed",
               )}
              >
  ...
  <div className="rounded-lg bg-surface-container-lowest p-md">
   <div className="flex items-start justify-between gap-4 flex-wrap">
    <div className="flex items-center gap-3 min-w-0">
     ...
     <div className="min-w-0">
      <h2 className="font-headline-md text-headline-md text-primary leading-none truncate">{student.name}</h2>
    ...
    <div className="flex items-center gap-2 shrink-0">
     ...
     {currentGuardian ? (
      <span className="inline-flex items-center gap-1 font-label-sm text-label-sm px-2 py-1 rounded-md bg-primary-fixed text-on-surface-variant font-medium">
       <span className="material-symbols-outlined text-[14px]">verified</span>
       Guardian linked
      </span>
     ) : (
      <span className="inline-flex items-center gap-1 font-label-sm text-label-sm px-2 py-1 rounded-md bg-error-container text-on-error-container font-medium">
