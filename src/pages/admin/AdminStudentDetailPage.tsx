 ...
 const tone =
  value >= 60
   ...
   : value >= 40
    ...
    : "bg-error-container text-on-error-container"
 return (
  <span className={cn("inline-flex items-center font-label-sm text-label-sm px-2 py-0.5 rounded-full font-semibold", tone)}>
   {value}%
  </span>
 )
}
    ...
    <div className="rounded-lg bg-surface-container-lowest p-md">
     <div className="flex items-start justify-between gap-4 flex-wrap">
      ...
      <div className="flex items-center gap-2 shrink-0 flex-wrap">
       ...
       {profile.guardian && (
        <span className="inline-flex items-center gap-1 font-label-sm text-label-sm px-2 py-1 rounded-full bg-primary-fixed text-on-surface-variant font-semibold">
       ...
       <Button
        variant="outline"
        size="sm"
        className="rounded-full font-label-md text-label-md"
    ...
    <Tabs defaultValue="grades">
     <div className="rounded-lg bg-surface-container-lowest p-1.5">
      <TabsList className="bg-surface-container-low h-auto p-1 w-full">
       <TabsTrigger value="grades" className="flex-1 rounded-full font-label-md text-label-md text-on-surface-variant data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-card">
        Quiz grades
       </TabsTrigger>
       ...
       <TabsTrigger value="fees" className="flex-1 rounded-full font-label-md text-label-md text-on-surface-variant data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-card">
        Fees
       </TabsTrigger>
      </TabsList>
     </div>
...
function HistoryPanel({ historyQuery, isLoading, isError, onRetry }: { historyQuery: api.StudentHistory | null; isLoading: boolean; isError: boolean; onRetry: () => void }) {
 ...
 return (
  <div className="space-y-4">
   {years.map((y) => (
    <div key={y.year} className="rounded-lg bg-surface-container-lowest overflow-hidden">
     <div className="px-md py-3 border-b border-outline-variant flex items-center justify-between gap-3 flex-wrap">
      <h3 className="font-headline-sm text-headline-sm text-primary">{y.year}</h3>
      <div className="flex items-center gap-3">
       ...
       {y.quizAveragePct !== null && (
        <span className={cn("inline-flex items-center rounded-full font-label-sm text-label-sm px-2 py-0.5 font-semibold",
         y.quizAveragePct >= 60 ? "bg-success-container text-on-success-container" : y.quizAveragePct >= 40 ? "bg-secondary-fixed text-on-secondary-fixed-variant" : "bg-error-container text-on-error-container")}>
         {y.quizAveragePct}% avg
        </span>
       )}
       {y.warnings.length > 0 && (
        <span className="inline-flex items-center gap-1 font-label-sm text-label-sm px-2 py-1 rounded-full bg-error-container text-on-error-container font-semibold">
     ...
     {y.warnings.length > 0 && (
      <div className="border-b border-outline-variant bg-error-container/20">
       {y.warnings.map((w) => (
        <div key={w.id} className="flex items-center gap-3 px-md py-2.5">
         <span className="material-symbols-outlined text-[16px] text-on-error-container shrink-0">warning</span>
         ...
         <span className={cn("shrink-0 inline-flex items-center rounded-full font-label-sm text-label-sm px-2 py-0.5 font-semibold", severityTone(w.severity ?? "MEDIUM"))}>
          {w.severity ?? "MEDIUM"}
         </span>
        </div>
       ))}
       ...
       <div className="flex-1 min-w-0">
        ...
        <p className="font-label-sm text-label-sm text-on-surface-variant mt-0.5 truncate">
         ...
         {d.aiSuggestedCategory && d.aiSuggestedCategory !== d.category ? (
          <span className="ml-2 inline-flex items-center gap-1 rounded-full bg-tertiary-fixed px-1.5 py-0.5 font-label-sm text-label-sm text-on-tertiary-fixed">
       ...
       {data.map((f) => (
        <TableRow key={f.id}>
         ...
         <TableCell className="px-3 py-2.5 whitespace-nowrap">
          <span className={cn("inline-flex items-center rounded-full font-label-sm text-label-sm px-2 py-0.5 font-semibold", feeStatusStyles[f.status])}>{f.status}</span>
         </TableCell>
         ...
         <TableCell className="px-3 py-2.5 text-right whitespace-nowrap">
          <div className="flex items-center justify-end gap-1">
           <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full text-on-surface-variant" onClick={() => { setEditing(f); setEditorOpen(true) }}>
            <span className="material-symbols-outlined text-[18px]">edit</span>
           </Button>
           <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full text-error" onClick={() => setDeletingFee(f)}>
            <span className="material-symbols-outlined text-[18px]">delete</span>
           </Button>
          </div>
         </TableCell>
