    ...
    if (result.failed.length > 0) {
     ...
     toast.error(
      result.failed.length === 1
       ? `"${first.fileName}" could not be uploaded`
       : `${result.failed.length} files could not be uploaded`,
      {
...
const handleFiles = (files: FileList | null) => {
  if (!files) return
  const accepted = Array.from(files).filter((f) => /\.(pdf|png|jpg|jpeg|webp|txt)$/i.test(f.name))
  if (accepted.length === 0) {
   toast.error("Only PDF, image or text files are accepted")
   return
  }
  const oversized = accepted.filter((f) => f.size > MAX_FILE_BYTES)
  const ok = accepted.filter((f) => f.size <= MAX_FILE_BYTES)
  if (oversized.length > 0) {
   toast.error(`${oversized.length} file${oversized.length === 1 ? "" : "s"} skipped — over the 20MB limit: ${oversized.map((f) => f.name).join(", ")}`)
  }
 ...
 return (
  <div className="px-6 pb-10">
   ...
   <section className="rounded-xl bg-surface-container-lowest border border-border p-md mb-6">
    <h2 className="font-label-md text-label-md text-primary font-medium mb-1">1 · Upload files</h2>
   ...
   <section className="rounded-xl bg-surface-container-lowest border border-border p-md">
    <h2 className="font-label-md text-label-md text-primary font-medium mb-1">2 · Review &amp; confirm</h2>
    <p className="font-body-sm text-body-sm text-on-surface-variant mb-4">
