...
const salaryStatusStyles: Record<api.SalaryStatus, string> = {
 ...
 POSTPONED: "bg-surface-container-high text-on-surface-variant",
  ...
  <div className="flex-1 px-4 py-4 min-w-0">
   <div className="max-w-[1100px] mx-auto space-y-4 min-w-0">
     ...
     <div className="flex items-start justify-between gap-4 flex-wrap">
      <div className="flex items-center gap-3 min-w-0">
       ...
       <div className="min-w-0">
        <h2 className="font-headline-md text-headline-md text-primary leading-none truncate">{profile.name}</h2>
        <p className="font-body-sm text-body-sm text-on-surface-variant mt-1 truncate">{profile.email}</p>
       </div>
      </div>
      <div className="flex items-center gap-2 shrink-0 flex-wrap">
       <span className="inline-flex items-center font-label-sm text-label-sm px-2 py-1 rounded-full bg-primary-fixed text-on-primary-fixed-variant font-semibold">
        TEACHER
       </span>
       {profile.gender && (
        <span className="inline-flex items-center gap-1 font-label-sm text-label-sm px-2 py-1 rounded-full bg-surface-container-low text-on-surface font-semibold">
       ...
       {profile.grades.slice(0, 3).map((g) => (
        <span key={g.id} className="inline-flex items-center font-label-sm text-label-sm px-2 py-1 rounded-full bg-surface-container-low text-on-surface font-semibold">
         {g.name ?? `Grade ${g.level}`}
        </span>
       ))}
       {profile.grades.length > 3 && (
        <span className="inline-flex items-center font-label-sm text-label-sm px-2 py-1 rounded-full bg-surface-container-low text-on-surface font-semibold">
    ...
    <Tabs defaultValue="classes">
     <div className="rounded-lg bg-surface-container-lowest p-1.5">
      <TabsList className="bg-surface-container-low h-auto p-1 w-full">
       <TabsTrigger value="classes" className="flex-1 rounded-full font-label-md text-label-md text-on-surface-variant data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-card">
        Current classes
       </TabsTrigger>
       <TabsTrigger value="history" className="flex-1 rounded-full font-label-md text-label-md text-on-surface-variant data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-card">
        Teaching history
       </TabsTrigger>
       <TabsTrigger value="salary" className="flex-1 rounded-full font-label-md text-label-md text-on-surface-variant data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-card">
        Salary
       </TabsTrigger>
       <TabsTrigger value="documents" className="flex-1 rounded-full font-label-md text-label-md text-on-surface-variant data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-card">
        Documents
       </TabsTrigger>
      </TabsList>
     </div>
...
function GradeChips({ grades }: { grades: { id: string; level: number; name: string | null }[] }) {
 ...
 return (
  <div className="flex flex-wrap gap-1">
   {grades.map((g) => (
    <span key={g.id} className="inline-flex items-center font-label-sm text-label-sm px-1.5 py-0.5 rounded-full bg-primary-fixed text-on-primary-fixed-variant font-semibold">
     {gradeName(g)}
    </span>
   ))}
  </div>
       ...
       {data.map((h) => (
        <TableRow key={h.id}>
         ...
         <TableCell className="px-3 py-2.5 text-right whitespace-nowrap">
          {h.active ? (
           <span className="inline-flex items-center font-label-sm text-label-sm px-2 py-0.5 rounded-full bg-success-container text-on-success-container font-semibold">Active</span>
          ) : (
           <span className="inline-flex items-center font-label-sm text-label-sm px-2 py-0.5 rounded-full bg-surface-container-high text-on-surface-variant font-semibold">Ended</span>
          )}
         </TableCell>
        </TableRow>
       ))}
...
function GenderEditor({ gender, teacherId }: { gender: api.TeacherGender | null; teacherId: string }) {
 ...
 return (
  <>
   <Button variant="outline" size="sm" className="rounded-full font-label-md text-label-md" onClick={() => { setValue(gender ?? "UNSET"); setOpen(true) }}>
   ...
   <Dialog open={open} onOpenChange={(next) => { if (!next && !saveM.isPending) setOpen(false) }}>
    <DialogContent className="rounded-lg max-w-sm bg-popover">
     <DialogHeader>
     ...
     <div>
      ...
      <Select value={value} onValueChange={(v) => setValue(v as api.TeacherGender | "UNSET")}>
       <SelectTrigger className="mt-1.5 h-auto rounded-full bg-surface-container-lowest px-md py-2 font-medium text-body-md focus:outline-none focus-visible:ring-transparent focus-visible:ring-offset-0">
        <SelectValue />
       </SelectTrigger>
       <SelectContent>
...
function DocumentsPanel({
 ...
 return (
  <PanelCard
   ...
   action={
    <Button variant="outline" size="sm" className="rounded-full font-label-md text-label-md" onClick={() => setUploadOpen(true)}>
  ...
  >
   {data.length === 0 ? (
    ...
    <div className="divide-y divide-border">
     {data.map((d) => (
      <div key={d.id} className="flex items-start gap-3 px-md py-3">
       ...
       <div className="flex items-center gap-1 shrink-0">
        <Button
         variant="ghost"
         size="icon"
         className="h-8 w-8 rounded-full text-on-surface-variant"
         title="Download"
         onClick={async () => {
          try {
        ...
        <Button
         variant="ghost"
         size="icon"
         className="h-8 w-8 rounded-full text-error"
         title="Delete"
...
function UploadDocumentDialog({ open, onOpenChange, teacherId }: { open: boolean; onOpenChange: (v: boolean) => void; teacherId: string }) {
 ...
 return (
  <Dialog open={open} onOpenChange={(next) => {
   ...
   <DialogContent className="rounded-lg max-w-lg bg-popover">
    <DialogHeader>
    ...
    <div className="space-y-4">
     ...
     <div>
      ...
      <Input
       ...
       className="mt-1.5 rounded-full bg-surface-container-lowest font-body-md text-body-md"
      />
     </div>
...
function SalaryPanel({
 ...
 return (
  <PanelCard
   ...
   action={
    <Button variant="outline" size="sm" className="rounded-full font-label-md text-label-md" onClick={() => { setEditing(null); setEditorOpen(true) }}>
       ...
       {data.map((r) => (
        <TableRow key={r.id}>
         ...
         <TableCell className="px-3 py-2.5 whitespace-nowrap">
          <span className={cn("inline-flex items-center rounded-full font-label-sm text-label-sm px-2 py-0.5 font-semibold", salaryStatusStyles[r.status])}>{r.status}</span>
         </TableCell>
         ...
         <TableCell className="px-3 py-2.5 text-right whitespace-nowrap">
          <div className="flex items-center justify-end gap-1">
           <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full text-on-surface-variant" onClick={() => { setEditing(r); setEditorOpen(true) }}>
            <span className="material-symbols-outlined text-[18px]">edit</span>
           </Button>
           <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full text-error" onClick={() => setDeleting(r)}>
            <span className="material-symbols-outlined text-[18px]">delete</span>
           </Button>
          </div>
         </TableCell>
...
function SalaryEditorDialog({
 ...
 return (
  <Dialog open={open} onOpenChange={(next) => {
   ...
   <DialogContent className="rounded-lg max-w-md bg-popover">
    <DialogHeader>
    ...
    <div className="space-y-4">
     <div>
      ...
      <Input
       ...
       className="mt-1.5 rounded-full bg-surface-container-lowest font-medium text-body-md"
      />
     </div>
 ...
 return (
  <div className="rounded-lg bg-surface-container-lowest p-md">
   <div className="flex items-center justify-between gap-3 flex-wrap">
    <h3 className="font-headline-sm text-headline-sm text-primary">Personal info</h3>
    <div className="flex items-center gap-2 flex-wrap">
     <Button variant="outline" size="sm" className="rounded-full font-label-md text-label-md" onClick={() => fileInputRef.current?.click()}>
     ...
     />
     <Button variant="outline" size="sm" className="rounded-full font-label-md text-label-md" onClick={() => setEditOpen(true)}>
      <span className="material-symbols-outlined text-[16px]">edit</span>
      Edit details
     </Button>
    </div>
   ...
   <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4 mt-4">
    ...
    <div className="flex items-start gap-2 min-w-0">
     ...
     <div className="flex items-center gap-1 shrink-0">
      {profile.ssnMasked && (
       <Button variant="ghost" size="sm" className="rounded-full font-label-md text-label-md" onClick={() => (revealedSsn ? setRevealedSsn(null) : revealM.mutate())}>
        <span className="material-symbols-outlined text-[16px]">{revealedSsn ? "visibility_off" : "visibility"}</span>
       </Button>
      )}
      <Button variant="ghost" size="sm" className="rounded-full font-label-md text-label-md" onClick={() => setSsnOpen(true)}>
       <span className="material-symbols-outlined text-[16px]">edit</span>
      </Button>
     </div>
    </div>
...
function PersonalDetailsDialog({
 ...
 return (
  <Dialog open={open} onOpenChange={(next) => { if (!next && !saveM.isPending) onOpenChange(false) }}>
   <DialogContent className="rounded-lg max-w-lg bg-popover max-h-[85vh] overflow-y-auto">
    <DialogHeader>
    ...
    <div className="space-y-4">
     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div>
       ...
       <Input type="date" value={dateOfBirth} onChange={(e) => setDateOfBirth(e.target.value)} className="mt-1.5 rounded-full bg-surface-container-lowest font-medium text-body-md" />
      </div>
     </div>
     ...
     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div>
       ...
       <Input type="email" value={personalEmail} onChange={(e) => setPersonalEmail(e.target.value)} className="mt-1.5 rounded-full bg-surface-container-lowest font-medium text-body-md" />
      </div>
     </div>
