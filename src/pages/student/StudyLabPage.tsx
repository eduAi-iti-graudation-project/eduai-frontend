 ...
 return (
  <div className="flex-1 flex flex-col">
   ...
   <div className="px-6 pb-6 flex-1">
    <div className="rounded-lg border border-border bg-surface-container-lowest p-md space-y-4">
     ...
     <div className="flex flex-wrap items-center gap-3">
      {kind === "PODCAST" && (
       <Select value={preset} onValueChange={(v) => setPreset(v as api.StudyLabPreset)}>
        <SelectTrigger className="form-input-focus rounded-full bg-surface px-3 py-2 text-label-md text-on-surface w-auto min-w-[150px]">
         <SelectValue />
        </SelectTrigger>
        <SelectContent>
         {presetOptions.map((p) => (
     ...
     <div className="flex gap-3">
      <input
       ...
       className="flex-1 form-input-focus rounded-lg bg-surface px-3 py-2 font-body-md text-body-md text-on-surface placeholder:text-on-surface-variant/60"
      />
  ...
  <div className="space-y-4">
   {generation.status === "PROCESSING" && (
    <div className="rounded-lg border border-highlight/40 bg-highlight/10 p-4">
     <p className="font-label-md text-label-md text-highlight flex items-center gap-2">
   ...
   {generation.status === "FAILED" && (
    <div className="rounded-lg border border-danger/30 bg-danger/10 p-4">
     <p className="font-label-md text-label-md text-danger">
      Generation failed: {generation.error ?? "unknown error"}
     </p>
    </div>
   )}
   {generation.status === "READY" && generation.payload && (
    <div className="rounded-lg border border-border bg-surface-container-lowest p-md">
