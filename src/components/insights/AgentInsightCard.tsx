...
const [expanded, setExpanded] = useState(false)
  ...
  return (
    <div className={cn("flex items-start gap-3", bare ? "" : "rounded-lg bg-surface-container-lowest p-md border border-outline-variant", className)}>
      <span className="material-symbols-outlined text-[22px] text-primary shrink-0 mt-0.5">auto_awesome</span>
      <div className="min-w-0 flex-1">
        <h4 className="font-label-md text-label-md text-on-surface font-bold mb-1.5">{insight.title}</h4>
        {richBody}
        {isLong && (
          <button
            type="button"
