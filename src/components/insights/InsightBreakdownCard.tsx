 ...
 return (
  <div className="flex items-center gap-2.5 min-w-0">
   ...
   <div className="w-20 h-1.5 rounded-full bg-surface-container-high shrink-0 overflow-hidden">
    <div
     className={cn("h-full rounded-full", tone === "weak" ? "bg-error/70" : "bg-on-success-container/70")}
 ...
 return (
  <div className="min-w-0">
   <div className="flex items-center gap-2 flex-wrap mb-2">
    <span className={cn("inline-flex items-center gap-1 font-label-sm text-label-sm px-2 py-0.5 rounded-full font-semibold", severityChipClass(b.severity))}>
   ...
   {(weak.length > 0 || strengths.length > 0) && (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 rounded-md bg-surface-container-low/50 p-3 mb-2">
     {weak.length > 0 && (
      <div>
       <p className="font-label-sm text-label-sm text-on-surface-variant mb-1.5 flex items-center gap-1">
        <span className="material-symbols-outlined text-[15px] text-error">trending_down</span>
        Needs work
       </p>
     ...
     {strengths.length > 0 && (
      <div>
       <p className="font-label-sm text-label-sm text-on-surface-variant mb-1.5 flex items-center gap-1">
        <span className="material-symbols-outlined text-[15px] text-on-success-container">trending_up</span>
        Doing well
       </p>
       <div className="space-y-1.5">
