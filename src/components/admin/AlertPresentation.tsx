      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {alert.skillGapCount > 0 && (
            <span className="inline-flex items-center gap-1 font-label-sm text-label-sm px-1.5 py-0.5 rounded-[6px] bg-primary-fixed text-on-primary-fixed-variant font-medium">
              {alert.skillGapCount} skill{alert.skillGapCount === 1 ? "" : "s"} to target
            </span>
          )}
        </div>
        <p className="font-body-sm text-sm text-on-surface-variant mt-0.5 leading-relaxed [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:2] overflow-hidden">
          {alert.reason}
        </p>
      </div>
    </div>
...
export function SeverityPill({
  ...
  return (
    <span className={cn("inline-flex items-center gap-1 font-label-sm text-label-sm px-2 py-1 rounded-[6px] font-medium shrink-0", severityChipClass(severity))}>
      <span
        className={cn(
          "material-symbols-outlined text-[14px]",
          severity === "HIGH" ? "text-[#93000a]" : severity === "MEDIUM" ? "text-[#78350f]" : "text-[#41465c]",
        )}
      >
