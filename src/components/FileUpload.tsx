   ...
   <div className="block cursor-pointer" onClick={openPicker} onDragOver={(event) => {
    ...
    <div
     className={cn(
      ...
      isDragging
       ? "scale-105 border-secondary/50 bg-secondary/20 shadow-[0_0_50px_rgba(164,48,115,0.3)]"
       : "border-white/20 bg-gradient-to-br from-primary/10 via-primary-dark/10 to-secondary/10 hover:border-primary/50/50 hover:bg-background/10",
      disabled && "cursor-not-allowed opacity-70",
     )}
    >
     {value ? (
      <div className="flex items-center justify-center gap-8">
       <div className="flex h-24 w-24 items-center justify-center rounded-lg bg-gradient-to-br from-primary to-secondary shadow-2xl">
       ...
       <Button
        type="button"
        variant="outline"
        className="rounded-md border-2 border-white/20 bg-background/10 px-4 py-3 text-xs font-black uppercase tracking-[0.16em] text-white shadow-[0_10px_30px_rgba(0,0,0,0.18)] hover:bg-background/15"
        onClick={(event) => {
      ...
      <>
       <div className="flex justify-center">
        <div className="relative inline-flex h-20 w-20 items-center justify-center rounded-lg border-[3px] border-black bg-[#ffd84d] shadow-[6px_6px_0_0_#000]">
         <Upload className="h-10 w-10 animate-[bounce_2s_ease-in-out_infinite] text-primary" />
         <ArrowRightCircle className="absolute -right-2 -bottom-2 h-10 w-10 animate-spin text-secondary" />
        </div>
       </div>
