...
const adminItems: NavItem[] = [
 ...
 const navItems = role === "ADMIN" ? adminItems : role === "GUARDIAN" ? guardianItems : role === "STUDENT" ? studentItems : items
  ...
  <nav className="md:hidden fixed bottom-0 left-0 w-full z-40 flex justify-around items-center px-margin-mobile py-base pb-safe bg-surface-container-lowest shadow-lg border-t border-border">
   {navItems.map((item) => (
    <Link
     key={item.id}
     to={item.href}
     className={`press-scale flex flex-col items-center justify-center transition-colors duration-150 ease-premium px-3 py-1 ${
      activeItem === item.id
       ? "text-primary"
